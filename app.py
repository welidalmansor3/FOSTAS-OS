...
<style>
    ...
    .fostas-status-ok { color: #4caf50; }
    .fostas-status-bad { color: #ff4b4b; }
</style>
...
with st.sidebar:
    ...
    # Eskiden bir key eksikse kod sessizce SIMULATION MODE'a düşüyordu ve
    # kullanıcı neden gerçek kod gelmediğini anlamıyordu. Artık en tepede görünüyor.
    st.subheader("🔌 AI Engine Status")
    status = fostas.status
    for engine_name, key in [("Gemini", "gemini"), ("Z.AI", "zai"), ("Tripo3D", "tripo")]:
        info = status[key]
        if info["ok"]:
            st.markdown(f"<span class='fostas-status-ok'>●</span> {engine_name}: bağlı", unsafe_allow_html=True)
        else:
            st.markdown(
                f"<span class='fostas-status-bad'>●</span> {engine_name}: eksik — {info['error']}",
                unsafe_allow_html=True
            )
    st.markdown("---")
    ...
    if all_files:
        ...
        if st.button("📦 Download Full Project ZIP"):
            ...
            with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
                ...
                # NOT: project.godot artık kök dizine, diğer path'lerle
                # çakışmayacak şekilde en son yazılıyor.
...
# Sohbet Çubuğunun Hemen Üstündeki Araç Çubuğu
with st.container():
